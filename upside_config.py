...
def write_membrane_potential(sequence, potential_library_path, membrane_thickness, 
    ...
    print 'UNH_residues_included:\n',\
        ''.join((f.upper() if i in UNH_residues_included else f.lower()) for i,f in enumerate(fasta_one_letter)),'\n'
    print 'UCO_residues_included:\n',\
        ''.join((f.upper() if i in UCO_residues_included else f.lower()) for i,f in enumerate(fasta_one_letter)),'\n'
    ...
    create_array(grp, 'energy',     z_energy)
    grp.energy._v_attrs.z_min = z_lib_min
    grp.energy._v_attrs.z_max = z_lib_max
...
def main():
    ...
    parser.add_argument('--membrane-potential-unsatisfied-CO-hbond-residues',default=[], type=parse_segments,
            help='Residues with unsatisfied CO hydrogen bonds, which will be marked as XXX_UCO in --membrane-potential. ' +
                 'NOTE: residues with both unsatisfied NH and CO H-bonds will be inferred by --membrane-potential-unsatisfied-NH-hbond-residues ' + 
                 'and --membrane-potential-unsatisfied-CO-hbond-residues together, which will detonated as XXX_UNH_UCO in --membrane-potential.') 
