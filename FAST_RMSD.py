...
def calc_reward_score_rmsd(rmsds, cluster_center_indices, transition_count_mat, alpha=1, n_simulations=30):
    '''
    rmsds: (n_samples,)
    ...
    rmsd_max = rmsds[cluster_center_indices].max()
    rmsd_min = rmsds[cluster_center_indices].min()
    rmsd_max_min = rmsd_max - rmsd_min 
    ...
    rewards = np.array([ 
        ( (rmsd_max - rmsds[cluster_center_indices[i]]) / rmsd_max_min + alpha * (c_max - c[i]) / c_max_min ) 
        for i in range(n_clusters) ])
    ...
    print '=' * 50
    print '%6s %8s %12s %16s %8s %6s' % ('idx', '#cluster', 'RMSD_target', '#To other states', 'reward', 'sim')
    for i in range(n_clusters):
        print '%6i %8i %12.3f %16i %8.3f %6i' % (
                cluster_center_indices[i], 
                transition_count_mat[i][i],
                rmsds[cluster_center_indices[i]], 
                c[i],
                rewards[i], 
                sims[i])
...
def main():
    ...
    rewards, sims, c = calc_reward_score_rmsd(
                        rmsds=tar_rmsd_mat, 
    ...
    with open(args.output+'.dat', 'w') as f:
        print >> f, '%6s %8s %12s %16s %8s %6s' % ('idx', '#cluster', 'RMSD_target', '#To other states', 'reward', 'sim')
        for i in range(args.n_clusters):
            print >> f, '%6i %8i %12.3f %16i %8.3f %6i' % (
                cluster_center_indices[i], 
                transition_count_mat[i][i],
